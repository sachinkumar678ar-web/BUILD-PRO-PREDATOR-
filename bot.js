...
http.createServer((req, res) => { res.writeHead(200); res.end('BulidPro Master AI V26 Active!\n'); }).listen(process.env.PORT || 3000);
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        // 🔥 PRIVATE DM AUTO-RESULT (बोट खुद चेक करेगा और वॉलेट अपडेट करेगा)
        for (const chatId in recoveryUsers) {
            let user = recoveryUsers[chatId];
            if (user.state === 'WAITING_RESULT' && user.activeBet && user.activeBet.issue === latestId) {
                if (isWin || isJackpot) {
                    user.wallet += (user.activeBet.amount * 0.95); // प्रॉफिट
                    bot.sendSticker(chatId, stats.stickers.win || DEFAULT_STICKERS.win).catch(()=>{});
                } else {
                    user.wallet -= user.activeBet.amount; // लॉस
                    bot.sendSticker(chatId, stats.stickers.loss || DEFAULT_STICKERS.loss).catch(()=>{});
                }
                
                // रिजल्ट चेक होने के बाद कंडीशन देखो
                if (user.wallet >= user.target) {
                    ...
                    bot.sendMessage(chatId, `✅ <b>रिज़ल्ट आ गया!</b>\n💰 <b>New Wallet:</b> ₹${Math.round(user.wallet)} / 🎯 <b>Target:</b> ₹${Math.round(user.target)}\n\nअगले दांव के लिए क्लिक करें👇`, {
                        parse_mode:'HTML',
...
// ==========================================
// 💬 BUTTON-BASED PRIVATE RECOVERY SYSTEM (Auto)
// ==========================================
...
bot.on('message', (msg) => {
    ...
    if (user.state === 'ASK_LOSS' && !isNaN(text)) {
        ...
        bot.sendSticker(chatId, stats.stickers.predStart || DEFAULT_STICKERS.predStart).catch(()=>{}); 
...
// बटन क्लिक्स हैंडलर (अब विन/लॉस बटन नहीं है, बोट खुद चेक करेगा)
bot.on('callback_query', (query) => {
    const chatId = query.message.chat.id;
    const data = query.data;
    let user = recoveryUsers[chatId];
    
    if (!user || (user.state !== 'READY' && user.state !== 'WAITING_RESULT')) {
        return bot.answerCallbackQuery(query.id, {text: "Please /recovery first"});
    }
    ...
    if (data === 'next_pred') {
        ...
        if (user.state === 'WAITING_RESULT') {
            return bot.answerCallbackQuery(query.id, {text: "⏳ रिज़ल्ट का इंतज़ार करें...", show_alert: true});
        }
        
        // चेक करो कि चैनल में कोई एक्टिव प्रेडिक्शन है या नहीं
        if (currentPrediction && currentPrediction.predType !== "WAIT" && currentPrediction.chance >= 70) {
            const bets = getDynamicBet(currentPrediction.level, user.wallet);
            const nBetDisplay = bets.nBet > 0 ? `₹${bets.nBet}` : `₹0 (Low Bal)`;
            
            let msg = `🎯 <b>PRIVATE SIGNAL</b>\n\n🆔 <b>Period:</b> #${currentPrediction.issue.slice(-4)}\n🌟 <b>लगाओ:</b> ${currentPrediction.predType}\n💵 <b>अमाउंट:</b> ₹${bets.bBet}\n🪙 <b>नंबर (${currentPrediction.nums}):</b> ${nBetDisplay}\n\n<i>⏳ बोट खुद रिज़ल्ट चेक करके आपका बैलेंस अपडेट करेगा... इंतज़ार करें!</i>`;
            
            bot.sendMessage(chatId, msg, { parse_mode: 'HTML' });
            
            // यूज़र को 'वेटिंग' में डाल दो
            user.state = 'WAITING_RESULT';
            user.activeBet = { amount: bets.bBet, issue: currentPrediction.issue };
...
console.log("🚀 BULIDPRO Master AI (Auto Result Update) Booted Successfully...");
