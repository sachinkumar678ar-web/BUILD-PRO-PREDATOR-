...
// ==========================================
// ⚙️ CONFIGURATION (Live on Render)
// ==========================================
...
const RENDER_APP_URL = "https://build-pro-predator.onrender.com/"; // आपका लाइव URL
...
let pendingSticker = null; // डायनामिक स्टीकर सेट करने के लिए
...
let stats = {
    ...
    hourlyData: {}, 
    percentTracker: {}, // % के हिसाब से Win/Loss ट्रैक करेगा
    stickers: {
        ...
        streak_win: 'CAACAgUAAxkBAAERUlhqH_0T0DxkDLbkEndLKZTBr0xjtgAC_A8AAg4fAAFUetjUfT4IR1s7BA', // 5 Back to back
        streak_jackpot: 'CAACAgUAAxkBAAERUfVqH5wMtoYUc1szw66OAfQpEoVnlwAC-hQAAu0OiFSGFOobgNZJHDsE',
        gm: 'CAACAgUAAxkBAAE...', // गुड मॉर्निंग
        gn: 'CAACAgUAAxkBAAE...'  // गुड नाईट
    }
};
...
// 🧠 10-HOUR HISTORY BAD NUMBER SCANNER
...
// ==========================================
// 🧠 25-RULES IMAGE PATTERN SCANNER & PREDICTION
// ==========================================
function generatePrediction(nextId) {
    ...
    // Convert history to string format B / S (A and B in your image rules)
    let seq = "";
    for(let i = 0; i < 8; i++) { seq += (history[i].number >= 5 ? "B" : "S"); }
    // seq[0] is newest, seq[7] is oldest. Let's reverse it to match human reading (oldest to newest)
    let p = seq.split('').reverse().join(''); 

    let finalSelection = history[0].number >= 5 ? "BIG" : "SMALL"; // Default Trend Following
    let baseWinChance = 65; 

    // 🔥 IMAGE 25 RULES IMPLEMENTATION (Mapped Deeply)
    if (p.endsWith("BSBSBS") || p.endsWith("SBSBSB")) { finalSelection = p.endsWith("S") ? "BIG" : "SMALL"; baseWinChance = 88; } // Rule 1: AB AB AB
    else if (p.endsWith("BBS") || p.endsWith("SSB")) { finalSelection = p.endsWith("S") ? "SMALL" : "BIG"; baseWinChance = 90; } // Rule 2: AABB
    else if (p.endsWith("BBBS") || p.endsWith("SSSB")) { finalSelection = p.endsWith("S") ? "SMALL" : "BIG"; baseWinChance = 92; } // Rule 3: AAABBB
    else if (p.endsWith("BBBBS") || p.endsWith("SSSSB")) { finalSelection = p.endsWith("S") ? "SMALL" : "BIG"; baseWinChance = 94; } // Rule 4: AAAABBBB
    else if (p.endsWith("BBSBBS") || p.endsWith("SSBSSB")) { finalSelection = p.endsWith("S") ? "BIG" : "SMALL"; baseWinChance = 87; } // Rule 5: AAB AAB
    else if (p.endsWith("BBBBBB") || p.endsWith("SSSSSS")) { finalSelection = p.endsWith("S") ? "SMALL" : "BIG"; baseWinChance = 80; } // Rule 6: Dragon follow
    else if (p.endsWith("BSSBSS") || p.endsWith("SBBSSBB")) { finalSelection = p.endsWith("S") ? "BIG" : "SMALL"; baseWinChance = 85; } // Rule 7: ABB ABB
    else if (p.endsWith("BBBSBBBS") || p.endsWith("SSSBSBSS")) { finalSelection = p.endsWith("S") ? "BIG" : "SMALL"; baseWinChance = 89; } // Rule 8: AAAB AAAB
    else if (p.endsWith("BBSBB") || p.endsWith("SSBSS")) { finalSelection = p.endsWith("B") ? "SMALL" : "BIG"; baseWinChance = 93; } // Rule 12/16: Break Patterns
    else if (p.endsWith("BSB") || p.endsWith("SBS")) { finalSelection = p.endsWith("B") ? "SMALL" : "BIG"; baseWinChance = 78; } // Rule 21: Alternate Start
    
    // प्रतिशत (Percentage) घटाने का लॉजिक
    let isBadTrend = activeBadNumbers.includes(lastNum) || [0, 5].includes(lastNum);
    
    if (isBadTrend) {
        baseWinChance = getRandomNumber(51, 59); // ख़राब मार्केट = रिस्क वार्निंग
    } else {
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        // TRACKING ANALYTICS (% के हिसाब से)
        let pctRange = Math.floor(currentPrediction.chance / 5) * 5; // e.g. 86 -> 85
...
// ==========================================
// 📊 ADMIN ANALYTICS COMMAND (/analytics)
// ==========================================
...
setInterval(() => {
    ...
    if (now.getHours() === 0 && now.getMinutes() === 0 && !midnightPinSent) {
        ...
        stats.hourlyData = {}; 
        stats.percentTracker = {}; // दिन खत्म होने पर % ट्रैक रीसेट
...
console.log("🚀 Predator Bot with Dynamic Stickers & Analytics Active...");
