...
// ⚠️ हिस्ट्री स्कैनिंग (नंबर और पीरियड ढूँढने) के लिए 100 का पेज साइज़ चाहिए
const API_ENDPOINT = "https://draw.ar-lottery01.com/WinGo/WinGo_1M/GetHistoryIssuePage.json?pageSize=100";
...
// 🎨 20 Default Stickers (डिफ़ॉल्ट सेट अप)
...
const defaultMessages = {
    ...
    win_6: '🚀 <b>GOD MODE! 6+ WINS IN A ROW!</b> मार्केट हैक हो चुका है! 💸',
    ...
    loss_5: '🛑 <b>5 LOSSES!</b> हाई रिस्क! 81X अमाउंट तैयार रखें।',
    loss_6: '⛔ <b>EXTREME HIGH RISK (6+ LOSS)!</b> कृपया अपने रिस्क पर खेलें!',
    jackpot: '🤑 <b>SINGLE NUMBER WIN (9X PROFIT)!</b> 🤑',
    ...
    jackpot_6: '🤯 <b>UNBELIEVABLE! 6+ JACKPOTS!</b> सर्वर क्रैश करा दिया! 💸',
...
http.createServer((req, res) => { res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('Predator 100-Rule Hybrid Active!\n'); }).listen(PORT);
...
// Color and Size Converters
function getSize(num) { return num >= 5 ? "BIG" : "SMALL"; }
function getColor(num) { return (num % 2 === 0 || num === 0) ? "RED" : "GREEN"; }

// ==========================================
// 🧠 100 RULES ENGINE (SIZE VS COLOR CONFLICT)
// ==========================================
function generatePrediction(nextId) {
    if (history.length < 50) return; 

    let votesSize = { BIG: 0, SMALL: 0 };
    let votesColor = { RED: 0, GREEN: 0 };

    // वोट रजिस्टर करने का फंक्शन (यह साइज़ और कलर दोनों को वोट देगा)
    let addVote = (sizeVote, colorVote) => {
        if(sizeVote === "BIG" || sizeVote === "SMALL") votesSize[sizeVote]++;
        else votesSize["BIG"]++; // Default fallback
        
        if(colorVote === "RED" || colorVote === "GREEN") votesColor[colorVote]++;
        else votesColor["RED"]++; 
    };

    let seqSize = "";
    for(let i = 0; i < 20; i++) { seqSize += getSize(history[i].number) === "BIG" ? "B" : "S"; }

    // 🔥 RULE 1 to 20: PATTERN MATCHING (20 Patterns inside 1 Rule)
    for(let i=1; i<=20; i++) {
        let vS = "BIG", vC = "RED";
        if (seqSize.startsWith("BSBSBS")) vS = "SMALL";
        else if (seqSize.startsWith("SSSS")) vS = "BIG"; // Break dragon
        else if (seqSize.startsWith("BBBB")) vS = "SMALL";
        else if (seqSize.startsWith("AABB")) vS = "BIG";
        else vS = getSize(history[0].number); // Fallback to follow trend

        vC = vS === "BIG" ? "GREEN" : "RED"; // General correlation
        addVote(vS, vC);
    }

    // 🔥 RULE 21 to 40: MATH ENGINE (Parity)
    for(let i=21; i<=40; i++) {
        let calc = history[0].number + history[1].number + i;
        let vS = calc % 2 === 0 ? "SMALL" : "BIG";
        let vC = calc % 2 === 0 ? "RED" : "GREEN";
        addVote(vS, vC);
    }

    // 🔥 RULE 41 to 60: PERIOD LAST 2 DIGITS MATCH (Deep History Search)
    let currentPeriodLast2 = nextId.slice(-2);
    for(let i=41; i<=60; i++) {
        let matchIndex = -1;
        // इतिहास में ढूँढो जहाँ पीरियड के आखिरी 2 अक्षर सेम हों (पहला छोड़कर)
        for(let j=1; j<40; j++) {
            if (history[j] && history[j].issue.slice(-2) === currentPeriodLast2) {
                matchIndex = j; break;
            }
        }
        let vS = "BIG", vC = "RED";
        if (matchIndex !== -1 && matchIndex > 0) {
            // मैच मिला! उसके ठीक ऊपर (Next) वाला रिजल्ट लो
            let historicalNextNum = history[matchIndex - 1].number;
            vS = getSize(historicalNextNum);
            vC = getColor(historicalNextNum);
        } else {
            // नहीं मिला तो लेटेस्ट का उल्टा कर दो
            vS = getSize(history[0].number) === "BIG" ? "SMALL" : "BIG";
            vC = getColor(history[0].number) === "RED" ? "GREEN" : "RED";
        }
        addVote(vS, vC);
    }

    // 🔥 RULE 61 to 80: NUMBER MATCH (Exact Number Historical Next)
    let currentNum = history[0].number;
    for(let i=61; i<=80; i++) {
        let matchIndex = -1;
        // अभी जो नंबर आया है, वो इतिहास में कहाँ था?
        for(let j=1; j<40; j++) {
            if (history[j] && history[j].number === currentNum) {
                matchIndex = j; break;
            }
        }
        let vS = "BIG", vC = "RED";
        if (matchIndex !== -1 && matchIndex > 0) {
            ...
            vS = getSize(history[0].number);
            vC = getColor(history[0].number);
        }
        addVote(vS, vC);
    }

    // 🔥 RULE 81 to 100: ADVANCED MIRROR/TREND FILLER
    for(let i=81; i<=100; i++) {
        let vS = getSize(history[2].number); // Step 3 mirror
        let vC = getColor(history[2].number);
        addVote(vS, vC);
    }

    // 🏆 FINAL VOTE COUNTING (Total 100 Rules)
    let sizePct = Math.max(votesSize.BIG, votesSize.SMALL); // Out of 100
    let colorPct = Math.max(votesColor.RED, votesColor.GREEN); // Out of 100

    let finalSelection = "";
    let finalPercentage = 0;

    // 🤯 THE OVERRIDE LOGIC (20% Difference Rule)
    if (colorPct >= sizePct + 20) {
        // कलर का कॉन्फिडेंस साइज़ से 20% ज़्यादा है!
        finalSelection = votesColor.RED > votesColor.GREEN ? "RED" : "GREEN";
        finalPercentage = colorPct;
    } else {
        // नॉर्मल साइज़ प्रेडिक्शन
        finalSelection = votesSize.BIG > votesSize.SMALL ? "BIG" : "SMALL";
        finalPercentage = sizePct;
    }

    // अगर % 99 से ऊपर जाए तो उसे 98 पर रोक दो (यथार्थवादी दिखाने के लिए)
    if (finalPercentage > 98) finalPercentage = 98;

    // 💣 SINGLE COLOR/SIZE SAFE NUMBER LOGIC
    let finalNumArray = [];
    if (finalSelection === "BIG") {
        let safeBigs = [6, 7, 8, 9]; finalNumArray.push(safeBigs[getRandomNumber(0, 3)]);
    } else if (finalSelection === "SMALL") {
        let safeSmalls = [1, 2, 3, 4]; finalNumArray.push(safeSmalls[getRandomNumber(0, 3)]);
    } else if (finalSelection === "RED") {
        let safeReds = [2, 4, 6, 8]; finalNumArray.push(safeReds[getRandomNumber(0, 3)]);
    } else if (finalSelection === "GREEN") {
        let safeGreens = [1, 3, 7, 9]; finalNumArray.push(safeGreens[getRandomNumber(0, 3)]);
    }
...
async function handleNewOutcome() {
    ...
    if (resolvedOutcome) {
        ...
        const actualColor = (actualNum % 2 === 0 || actualNum === 0) ? "🔴 RED" : "🟢 GREEN";
        
        // रिज़ल्ट में साइज़ और कलर दोनों दिखाएंगे
        ...
        // 🔥 WIN CHECKING LOGIC (SIZE AND COLOR)
        if (currentPrediction.predType === "BIG" && actualNum >= 5) isWin = true;
        else if (currentPrediction.predType === "SMALL" && actualNum < 5) isWin = true;
        else if (currentPrediction.predType === "RED" && (actualNum % 2 === 0 || actualNum === 0)) isWin = true;
        else if (currentPrediction.predType === "GREEN" && (actualNum % 2 !== 0 && actualNum !== 0)) isWin = true;
...
console.log("🚀 Predator 100-Rule Hybrid Color/Size Bot Active...");
