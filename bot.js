...
// 🔥 HOT NUMBERS LOGIC (जैकपॉट फिक्स)
function getHotNumbers(historyData, type) {
    ...
    // जो सबसे ज़्यादा बार आया है, उसे ऊपर रखो
    let sorted = Object.keys(counts).sort((a,b) => counts[b] - counts[a]);
    
    // टॉप 2 हॉट नंबर चुनें (इससे जैकपॉट का चांस डबल हो जाएगा)
    if (sorted.length >= 2) return [parseInt(sorted[0]), parseInt(sorted[1])];
    return type === "BIG" ? [7, 9] : [1, 3]; // Fallback
}
...
function generatePrediction(nextId) {
    ...
    // 🔥 HOT NUMBER PREDICTION (अब 2 लकी नंबर देगा)
...
async function sendPredictionToChannel() {
    ...
    // चूँकि 2 नंबर हैं, इसलिए nBet (EACH) होगा
...
bot.on('message', (msg) => {
    ...
    // 🔥 1. कोई भी यूज़र (Admin/Non-Admin) स्टीकर भेजेगा, तो उसे ID मिलेगी
    if (msg.sticker) {
        let sId = msg.sticker.file_id;
        bot.sendMessage(chatId, `🏷️ **Here is your Sticker ID:**\n\`${sId}\`\n\n*(Touch to copy)*`, { parse_mode: 'Markdown' });
        
        // अगर एडमिन है, तो उसे सेट करने का मेन्यू भी मिलेगा
    ...
    // 2. अगर एडमिन ने टेक्स्ट मैसेज भेजा है सेट करने के लिए
...
console.log("🚀 Ultimate Predator Bot Active...");
