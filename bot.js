...
http.createServer((req, res) => { res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('Predator 1-Number AI Active!\n'); }).listen(PORT);
...
// 🔥 HOT NUMBERS LOGIC (सिर्फ 1 नंबर)
function getHotNumbers(historyData, type) {
    ...
    if (sorted.length >= 1) return [parseInt(sorted[0])]; // केवल टॉप 1 नंबर देगा
    
    // अगर कोई डेटा नहीं मिला, तो सेफ रैंडम 1 नंबर देगा
    let safeBigs = [6, 7, 8, 9];
    let safeSmalls = [1, 2, 3, 4];
    return type === "BIG" ? [safeBigs[getRandomNumber(0, 3)]] : [safeSmalls[getRandomNumber(0, 3)]]; 
}
...
console.log("🚀 Ultimate 1-Number Predator Active...");
