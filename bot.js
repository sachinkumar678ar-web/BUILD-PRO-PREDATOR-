...
const defaultMessages = {
    ...
    loss_6: '⛔ <b>EXTREME HIGH RISK (6+ LOSS)!</b> अनलिमिटेड बैकअप इस्तेमाल करें।',
...
http.createServer((req, res) => { res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('Anti-Loss Trend Surfer Active!\n'); }).listen(PORT);
...
// ==========================================
// 🧠 ANTI-LOSS TREND SURFER AI (No Level 9)
// ==========================================
function generatePrediction(nextId) {
    if (history.length < 10) return; 

    // s0 = Last result, s1 = Second Last, s2 = Third Last
    let s0 = history[0].number >= 5 ? "B" : "S"; 
    let s1 = history[1].number >= 5 ? "B" : "S";
    let s2 = history[2].number >= 5 ? "B" : "S"; 
    let s3 = history[3].number >= 5 ? "B" : "S";
    let s4 = history[4].number >= 5 ? "B" : "S";

    let seq = s4 + s3 + s2 + s1 + s0; // Oldest to Newest

    let finalSelection = "BIG";
    let baseWinChance = 85;

    // 🔥 NEVER FIGHT THE TREND LOGIC (ड्रैगन को राइड करो, तोड़ो मत)
    if (s2 === "B" && s1 === "B" && s0 === "B") {
        finalSelection = "BIG"; // Ride the BIG dragon
        baseWinChance = 94;
    } 
    else if (s2 === "S" && s1 === "S" && s0 === "S") {
        finalSelection = "SMALL"; // Ride the SMALL dragon
        baseWinChance = 94;
    } 
    // 🔥 CHOP RIDER LOGIC (कचरा मार्केट में उल्टा दांव)
    else if (s2 === "B" && s1 === "S" && s0 === "B") {
        finalSelection = "SMALL"; // Catch the chop
        baseWinChance = 89;
    } 
    else if (s2 === "S" && s1 === "B" && s0 === "S") {
        finalSelection = "BIG"; // Catch the chop
        baseWinChance = 89;
    } 
    // 🔥 DOUBLE PATTERN (AABB)
    else if (s3 === "B" && s2 === "B" && s1 === "S" && s0 === "S") {
        finalSelection = "BIG"; // Complete the double
        baseWinChance = 92;
    } 
    else if (s3 === "S" && s2 === "S" && s1 === "B" && s0 === "B") {
        finalSelection = "SMALL"; // Complete the double
        baseWinChance = 92;
    } 
    // 🔥 SHORT TRENDS
    else if (s1 === "B" && s0 === "B") {
    ...
    // 🔥 DEFAULT: FOLLOW THE LAST RESULT (सबसे सेफ तरीका)
    else {
        finalSelection = s0 === "B" ? "BIG" : "SMALL";
        baseWinChance = 78;
    }
    ...
    if (finalSelection === "BIG") {
        let safeBigs = [7, 9]; // सबसे सेफ नंबर
        finalNumArray.push(safeBigs[getRandomNumber(0, 1)]);
    } else {
        let safeSmalls = [1, 3]; // सबसे सेफ नंबर
        finalNumArray.push(safeSmalls[getRandomNumber(0, 1)]);
    }

    currentPrediction = { issue: nextId, predType: finalSelection, nums: finalNumArray, level: botLevel, messageId: null, isChannelPosted: false, predTime: getISTTimeString(), chance: baseWinChance };
...
async function sendPredictionToChannel() {
    ...
    let chanceIcon = currentPrediction.chance >= 85 ? "🔥" : "⚡";
...
async function handleNewOutcome() {
    ...
    if (resolvedOutcome) {
        ...
        if (isWin || isJackpot) {
            ...
            // 🔥 ADVANCED LOSS RECOVERY MESSAGE 🔥
            if (recoveredLosses >= 3) {
                let recMsg = isJackpot 
                    ? `🚀 <b>MEGA RECOVERY!</b> ${recoveredLosses} लगातार Loss के बाद सीधा <b>JACKPOT</b> उड़ाया! सारा लॉस कवर! 💸💣`
...
console.log("🚀 Anti-Loss Trend Surfer Active...");
