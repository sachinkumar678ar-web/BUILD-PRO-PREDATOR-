...
// 📊 5-DAY ROLLING STATS OBJECT
let stats = {
    total: 0, wins: 0, losses: 0, jackpots: 0,
    maxLevelReached: 1, levelHistoryTracker: {}, 
    hourlyData: {}, // आज का डेटा
    rollingHourlyData: {}, // 🔥 पिछले 5 दिनों का डेटा स्टोर करने के लिए
    percentTracker: {}
};
...
http.createServer((req, res) => { res.writeHead(200, { 'Content-Type': 'text/plain' }); res.end('Predator 5-Day Analytics Active!\n'); }).listen(PORT);
...
async function handleNewOutcome() {
    ...
    if (resolvedOutcome) {
        ...
        // 🔥 TRACKING HOURLY WINS & MAX LEVEL 
        ...
        if(botLevel > stats.hourlyData[currentHour].maxLevel) stats.hourlyData[currentHour].maxLevel = botLevel; // उस घंटे का मैक्स लेवल सेव करें
...
bot.onText(/\/analytics/, (msg) => {
    ...
    // 5 दिन का डेटा और आज का डेटा जोड़ना
    for(let h=0; h<24; h++) {
    ...
    // 🟢 SORT FOR BEST TIMES (High Rate, Low Max Level)
    ...
    // 🔴 SORT FOR DANGER TIMES (High Max Level reached in last 5 days)
...
setInterval(() => {
    ...
    if (now.getHours() === 0 && now.getMinutes() === 0 && !midnightPinSent) {
        ...
        for(let h=0; h<24; h++) {
            ...
            if(stats.rollingHourlyData[h].length > 5) stats.rollingHourlyData[h].shift(); // सिर्फ 5 दिन का डेटा रखें
        }
        
        stats.hourlyData = {}; // आज का डेटा खाली कर दें
...
console.log("🚀 The Ultimate 5-Day Tracker & 2000-Rule Quant Bot Active...");
